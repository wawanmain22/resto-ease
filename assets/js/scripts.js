...
function loadSettings() {
  ...
  const miniSidebar = localStorage.getItem("mini-sidebar");
  const stickyHeader = localStorage.getItem("sticky-header");

  if (layout) {
    $("body").removeClass("light dark").addClass(layout);
    $(`.select-layout[value|='${layout === "light" ? 1 : 2}']`).prop(
      "checked",
      true
    );
  }
  ...
  if (sidebarColor) {
    $("body").removeClass("light-sidebar dark-sidebar").addClass(sidebarColor);
    $(
      `.select-sidebar[value|='${sidebarColor === "light-sidebar" ? 1 : 2}']`
    ).prop("checked", true);
  }

  if (theme) {
    $("body").removeClass("theme-white theme-black").addClass(theme);
  ...
  if (miniSidebar) {
    toggle_sidebar_mini(miniSidebar === "true");
    $("#mini_sidebar_setting").prop("checked", miniSidebar === "true");
  }

  if (stickyHeader) {
    toggle_sticky_header(stickyHeader === "true");
    $("#sticky_header_setting").prop("checked", stickyHeader === "true");
  }
}

// Pindahkan fungsi ini ke luar blok jQuery
var toggle_sidebar_mini = function (mini) {
  ...
  if (!mini) {
    ...
    $(".main-sidebar").css({
      overflow: "hidden",
      width: "250px", // Reset to default width
    });
    setTimeout(function () {
    ...
    $(".main-sidebar").css({
      ...
      width: "65px", // Set mini sidebar width
    });
...
$(function () {
  ...
  var toggleLayout = function (layout) {
    var miniSidebar = $("body").hasClass("sidebar-mini"); // Store mini sidebar state
    ...
    saveSettings("layout", layout);
    saveSettings(
      "sidebar-color",
      layout === "light" ? "light-sidebar" : "dark-sidebar"
    );
    saveSettings("theme", layout === "light" ? "theme-white" : "theme-black");
  };

  var toggleLayoutHandler = function () {
    var w = $(window),
  ...
  toggleLayoutHandler();
  $(window).resize(toggleLayoutHandler);
  ...
  $("#mini_sidebar_setting").on("change", function () {
    ...
    if (_val === "checked") {
      toggle_sidebar_mini(true);
      saveSettings("mini-sidebar", "true");
    } else {
      toggle_sidebar_mini(false);
      saveSettings("mini-sidebar", "false");
    }
  ...
  $("#sticky_header_setting").on("change", function () {
    if ($(".main-navbar")[0].classList.contains("sticky")) {
      ...
      saveSettings("sticky-header", "false");
    } else {
      toggle_sticky_header(true);
      saveSettings("sticky-header", "true");
    }
  });
  ...
  $(".choose-theme li").on("click", function () {
    ...
    saveSettings("theme", "theme-" + $(this).attr("title"));
  });
  ...
  $(".sidebar-color input:radio").change(function () {
    if ($(this).val() == "1") {
      ...
      saveSettings("sidebar-color", "light-sidebar");
    } else {
      $("body").removeClass("light-sidebar");
      $("body").addClass("dark-sidebar");
      saveSettings("sidebar-color", "dark-sidebar");
    }
  });
  ...
  $(".btn-restore-theme").on("click", function () {
    ...
    saveSettings("layout", "light");
    saveSettings("theme", "theme-white");
    saveSettings("sidebar-color", "light-sidebar");
    saveSettings("mini-sidebar", "false");
    saveSettings("sticky-header", "true");
  });
